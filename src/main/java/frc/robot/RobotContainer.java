...
public class RobotContainer {
  ...
  JoystickButton xboxRightBumper = new JoystickButton(SecondaryController, Button.kBumperRight.value);  // Right Bumper
  ...
  Command runConveyorUpperIn = new RunCommand(() -> Apollo_ConveyorUpper.runConveyorUpper("In", 0.5*SecondaryController.getTriggerAxis(Hand.kLeft)), Apollo_ConveyorUpper);
  Command feedBallsToShooter = new ConditionalCommand(runConveyorUpperIn, stopConveyorUpper, () -> SecondaryController.getTriggerAxis(Hand.kLeft) > ConveyorConstants.kConveyorFeedDeadband);
  ...
  Command runSweeperIn = new RunCommand(() -> Apollo_Intake.sweep("In", SecondaryController.getTriggerAxis(Hand.kRight)), Apollo_Intake);
  ParallelCommandGroup runBallsIn = new ParallelCommandGroup(runConveyorLowerIn, runSweeperIn);
  Command intakeBalls = new ConditionalCommand(runBallsIn, stopLowerConveyorAndSweeper, () -> SecondaryController.getTriggerAxis(Hand.kRight) > ConveyorConstants.kConveyorFeedDeadband );
